...
export default function AccountScreen(){
  ...
  if(!user){
    return <SafeAreaView style={[styles.safe,{backgroundColor:c.background}]}>
      ...
      <View style={styles.empty}><Text style={[styles.title,{color:c.text}]}>You're not signed in</Text><Text style={[styles.subtitle,{color:c.textMuted}]}>Sign in to sync preferences, memory and history across devices.</Text><Pressable onPress={()=>router.replace("/auth")} style={[styles.primary,{backgroundColor:c.text}]}><Text style={[styles.primaryText,{color:"#fff"}]}>Sign in or create account</Text></Pressable></View>
    </SafeAreaView>;
  }

  async function signOut(){ if(supabase){await supabase.auth.signOut(); router.back();} }
  ...
  return <SafeAreaView style={[styles.safe,{backgroundColor:c.background}]}>
    ...
    <View style={styles.body}>
      ...
      <View style={[styles.list,{backgroundColor:c.surface,borderColor:c.border}]}>
        ...
        <View style={[styles.row,{borderBottomColor:c.border}]}><Ionicons name="shield-checkmark-outline" size={20} color={c.text}/><Text style={[styles.rowText,{color:c.text}]}>Account security</Text><Ionicons name="chevron-forward" size={18} color={c.textMuted}/></View>
        <View style={[styles.row,{borderBottomColor:c.border}]}><Ionicons name="download-outline" size={20} color={c.text}/><Text style={[styles.rowText,{color:c.text}]}>Export account data</Text><Ionicons name="chevron-forward" size={18} color={c.textMuted}/></View>
      </View>
      ...
      <Pressable onPress={()=>Alert.alert("Delete account","Account deletion will require server-side confirmation and re-authentication before release.")} style={styles.delete}><Text style={[styles.deleteText,{color:c.textMuted}]}>Delete account</Text></Pressable>
    </View>
  </SafeAreaView>;
}
const styles=StyleSheet.create({
 ...
 danger:{height:54,borderWidth:1,borderRadius:27,marginTop:20,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:8},dangerText:{fontSize:14,fontWeight:"800"},delete:{padding:18,alignItems:"center"},deleteText:{fontSize:12}
});
